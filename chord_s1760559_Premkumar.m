...
function y = chord_s1760559_Premkumar(number,gap,rho,R,dur)
    ...
    switch number
        case 1
            for n = 1:length(Am_chord)
                ...
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(Am_chord(n),rho,R,dur);
            end
        case 2
            for n = 1:length(C_chord)
               start_pos = round(1+(n-1)*gap*Fs);
               y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(C_chord(n),rho,R,dur);               
            end
        case 3
            for n = 1:length(G_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(G_chord(n),rho,R,dur);               
            end
        case 4
            for n = 1:length(Em_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(Em_chord(n),rho,R,dur);
            end
        case 5
            for n = 1:length(Dm_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(Dm_chord(n),rho,R,dur);
            end
            
        case 6    
            for n = 1:length(F_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(F_chord(n),rho,R,dur);
            end
        case 7
            for n = 1:length(Bm_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(Bm_chord(n),rho,R,dur);
            end
        case 8
            for n = 1:length(D_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(D_chord(n),rho,R,dur);
            end
        case 9
            for n = 1:length(Gm_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(Gm_chord(n),rho,R,dur);
            end
        case 10
            for n = 1:length(Bflat_chord)
                start_pos = round(1+(n-1)*gap*Fs);
                y(start_pos:start_pos+len-1) =  y(start_pos:start_pos+len-1)+guitar_s1760559_Premkumar(Bflat_chord(n),rho,R,dur);
            end
        otherwise
